	...
	const formatPlaceholder = () => {
		 ...
		 if (listing.type === ListingType.Auction){
			return Number(!minimumNextBid?.displayValue) === 0 ?
			 "Enter Bid Amount" : `${minimumNextBid?.displayValue}${minimumNextBid?.symbol}
			  or more`;
	...
	const  createBidOrOffer = async() => {
		...
		try{
			...
			if (listing?.type === ListingType.Direct){
				...
				console.log("Buyout is not met , making offer")
				await makeOffer({
					quantity : 1,
					listingId,
		...
		<main className='max-w-6xl mx-auto p-2 flex flex-col lg:flex-row space-y-10 space-x-5 pr-10'>
			...
			<section className='flex-1 space-y-5 pb-20 lg:pb-0'>
				...
				<div className='grid grid-cols-2 items-center py-2'> 
					...
					<button onClick = {buyNft} className='col-start-2 mt-2 rounded-full  bg-blue-600 font-bold text-white w-44 py-4 px-10'>
                       Buy Now
					</button>
				</div>
				...
				<div className='grid grid-cols-2 space-y-2 items-center justify-end'>
				   ...
				   {listing.type === ListingType.Auction && (
					<>
					 ...
					 <p>Time Remaining</p>
					 <p><Countdown date = {Number(listing.endTimeInEpochSeconds.toString()) * 1000} /></p>
					</>
				   )}
